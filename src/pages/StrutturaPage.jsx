...
// Build Tree from flat list of users (Clean)
function buildUserTree(rootUser, allUsers, isRootNode = true) {
    if (!rootUser) return null;

    const rootId = String(rootUser.uid || rootUser.id || "").trim();
    if (!rootId) return null;

    const children = allUsers
        .filter(u => String(u.driverUid || "").trim() === rootId) // Find direct recruits
...
export default function NuovaStrutturaPage() {
    ...
    useEffect(() => {
        ...
        async function fetchStructure() {
            setLoading(true);
            try {
                // 1. Fetch ALL users (Robust method for tree consistency)
                // We fetch all users and let the recursive builder reconstruction the tree.
                // This avoids missing nodes if driverChain is imperfect or indexes lag.
                const q = query(collection(db, "users"));
                const snap = await getDocs(q);
                const allDownlineRaw = snap.docs.map(d => ({ uid: d.id, ...d.data() }));

                // Filter out obviously irrelevant users (optional, but good for safety)
                // Actually, passing everyone to buildUserTree is safEST.

                console.log("[Struttura] My UID:", user.uid);
                console.log("[Struttura] Total users fetched:", allDownlineRaw.length);
                const downline = allDownlineRaw;
