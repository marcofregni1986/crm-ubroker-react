...
// ✅ Tool to insert HTML tags
function insertFormat(ref, startTag, endTag, onUpdate, currentVal) {
  ...
  const newVal = `${before}${startTag}${sel}${endTag}${after}`;
  onUpdate(newVal);
...
function FormattingToolbar({ targetRef, onUpdate, value }) {
  return (
    <div style={{ display: "flex", gap: 5, marginBottom: 5 }}>
      <button type="button" className="btn-secondary" style={{ fontWeight: "bold", padding: "2px 8px", minHeight: 28 }} onClick={() => insertFormat(targetRef, "<b>", "</b>", onUpdate, value)}>B</button>
      <button type="button" className="btn-secondary" style={{ fontStyle: "italic", padding: "2px 8px", minHeight: 28 }} onClick={() => insertFormat(targetRef, "<i>", "</i>", onUpdate, value)}>I</button>
      <button type="button" className="btn-secondary" style={{ textDecoration: "underline", padding: "2px 8px", minHeight: 28 }} onClick={() => insertFormat(targetRef, "<u>", "</u>", onUpdate, value)}>U</button>
      <div style={{ width: 1, background: "var(--uni-border)", margin: "0 4px" }} />
      <button type="button" className="btn-secondary" style={{ color: "#ef4444", padding: "2px 8px", minHeight: 28 }} onClick={() => insertFormat(targetRef, '<span style="color: #ef4444">', "</span>", onUpdate, value)}>Red</button>
      <button type="button" className="btn-secondary" style={{ color: "#10b981", padding: "2px 8px", minHeight: 28 }} onClick={() => insertFormat(targetRef, '<span style="color: #10b981">', "</span>", onUpdate, value)}>Green</button>
      <div style={{ width: 1, background: "var(--uni-border)", margin: "0 4px" }} />
      <button type="button" className="btn-secondary" style={{ fontSize: 11, padding: "2px 8px", minHeight: 28 }} onClick={() => insertFormat(targetRef, "<br>", "", onUpdate, value)}>BR</button>
    </div>
  );
}
...
export default function University() {
  ...
  // Pagina accessibile se ha canAccessUniversity (default true)
  const canSeeUniversityPage = isAdmin || !!perms.canManageUniversity || !!perms.canAccessUniversity;
  // Contenuti privati accessibili solo se admin o esplicitamente gestore
  const canSeeAllContent = isAdmin || !!perms.canManageUniversity;
  ...
  const [refreshKey, setRefreshKey] = useState(0); // 🔥 Per sync manuale
  ...
  const lessonDescRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        // ✅ Filtraggio Permessi per utenti NON-admin/manager
        // Logica: vede solo moduli esplicitamente assegnati (ID o Topic)
        // + sempre "I PRIMI PASSI DEL COLLABORATORE" come base
        if (!canSeeAllContent) {
          ...
          items = items.filter(m => {
            ...
            // 2. Normalizzazione Topic
            const raw = (m.topic || "").trim().toLowerCase();
            const topic = (!raw || raw === "generale" || raw === "altro")
              ? "(senza sezione)"
              : raw;

            // 3. ✅ ECCEZIONE GLOBALE: "I PRIMI PASSI" sempre visibile a tutti
            if (topic === "i primi passi del collaboratore") return true;

            // 4. ❌ ESCLUSIONE: moduli senza sezione NON visibili a utenti base
            // (possono essere sbloccati solo tramite allowedModuleIds)
            if (topic === "(senza sezione)") return false;

            // 5. Controllo per Topic assegnato
    ...
    return () => unsub();
  }, [db, firebaseUser?.uid, loading, canSeeAllContent, JSON.stringify(perms.allowedTopics || []), JSON.stringify(perms.allowedModuleIds || []), refreshKey]);
  ...
  useEffect(() => {
    ...
    return () => unsub();
  }, [db, firebaseUser?.uid, loading, activeModuleId, canSeeAllContent]);
    ...
    STATIC_FOLDERS.forEach(label => {
      ...
      if (!isPresent) {
        ...
        if (canSeeAllContent) {
          ...
          ["i primi passi del collaboratore", "certificazioni e riconoscimenti"].includes(label.toLowerCase())) {
          result.push({ label: label, count: 0, previewImages: [] });
        }
      }
    });
  ...
  }, [modules, canSeeAllContent, JSON.stringify(perms.allowedTopics || []), topicMetadata]);
  ...
  const showNoAccess = !loading && !!firebaseUser?.uid && !canSeeAllContent && !modulesLoading && modules.length === 0;
    ...
    <div className="main">
      ...
      <div className="main-header" style={{ alignItems: "flex-start" }}>
        ...
        <div className="main-header-right" style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div
            ...
            title={canSeeAllContent ? "Puoi gestire l'intera University" : "Stai visualizzando i moduli che ti sono stati assegnati"}
          >
            {canSeeAllContent ? <ShieldCheck size={16} /> : <Lock size={16} />}
            {canSeeAllContent ? "Accesso di gestione" : "Accesso controllato"}
          </div>
          ...
          {activeModuleId ? (
            ...
            <button
              ...
              onClick={() => {
                ...
                alert("Contenuti sincronizzati!");
              }}
              style={{ padding: "0 12px", height: 46, borderRadius: 14 }}
            >
              <RefreshCcw size={18} className={modulesLoading ? "spin-icon" : ""} />
            </button>
          )}
        </div>
      </div>
                      ...
                      return (
                        <div key={l.id} style={{ borderTop: "1px solid var(--uni-border)", paddingTop: 32 }}>
                          ...
                          <div className="uni-lessonBody">
                            ...
                            {imgs.length > 0 && (
                              <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(360px, 1fr))", gap: 16 }}>
                                {imgs.map((url, i) => (
                                  <div
                                    key={i}
                                  ...
                                  >
                                    ...
                                    <div style={{
                                      ...
                                      display: "grid", placeItems: "center", transition: "opacity 0.2s"
                                    }}
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12 }}>
          ...
          <div className="form-group">
            ...
            <FormattingToolbar targetRef={moduleDescRef} onUpdate={(v) => setModuleDraft(p => ({ ...p, description: v }))} value={moduleDraft.description} />
            <textarea ref={moduleDescRef} className="form-input" rows={15} value={moduleDraft.description} onChange={(e) => setModuleDraft((p) => ({ ...p, description: e.target.value }))} placeholder="Descrizione breve del modulo..." />
          </div>
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12 }}>
          ...
          <div className="form-group">
            ...
            <FormattingToolbar targetRef={lessonDescRef} onUpdate={(v) => setLessonDraft(p => ({ ...p, description: v }))} value={lessonDraft.description} />
            <textarea ref={lessonDescRef} className="form-input" rows={15} value={lessonDraft.description} onChange={(e) => setLessonDraft((p) => ({ ...p, description: e.target.value }))} placeholder="Spiega cosa deve fare / capire l'utente..." />
          </div>
            ...
            {canManageUniversity ? (
              <>
                <div className="uni-uploadRow" style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  >
                    {imgUploading ? "Caricamento..." : "Carica foto (una o più)"}
                  </button>
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                        <img
                          src={url}
                          alt=""
                          style={{ width: 240, height: 180, borderRadius: 12, objectFit: "cover", border: "1px solid rgba(255,255,255,0.12)" }}
                        />
                        <a
                          href={url}
                          target="_blank"
      ...
      {imageViewer.open && (
        ...
        >
          ...
          >
            <div style={{ fontSize: 28, lineHeight: 1, paddingBottom: 2 }}>&times;</div>
          </button>
          ...
          >
            {imageViewer.urls.length > 1 && (
              <button
                style={{
                  position: "absolute", left: 20, background: "rgba(255,255,255,0.1)", border: 0, borderRadius: "50%",
                  width: 48, height: 48, color: "#fff", fontSize: 24, cursor: "pointer", display: "grid", placeItems: "center"
                }}
                onClick={(e) => {
                  e.stopPropagation();
                  setImageViewer(p => ({ ...p, index: (p.index - 1 + p.urls.length) % p.urls.length }));
                }}
              >
                <ChevronLeft size={24} />
              </button>
            ...
            <img
              ...
              alt=""
              style={{ maxHeight: "90vh", maxWidth: "90vw", objectFit: "contain", borderRadius: 4, boxShadow: "0 0 20px rgba(0,0,0,0.5)" }}
            />

            {imageViewer.urls.length > 1 && (
              <button
                style={{
                  position: "absolute", right: 20, background: "rgba(255,255,255,0.1)", border: 0, borderRadius: "50%",
                  width: 48, height: 48, color: "#fff", fontSize: 24, cursor: "pointer", display: "grid", placeItems: "center"
                }}
                onClick={(e) => {
                  e.stopPropagation();
                  setImageViewer(p => ({ ...p, index: (p.index + 1) % p.urls.length }));
                }}
              >
          ...
          <div style={{ height: 60, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.7)" }}>
