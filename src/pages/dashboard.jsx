...
// Lucide
import { Eye, StickyNote, X, Layers, Calendar, Clock, Target, Activity, Phone, RefreshCw, Trash2, Layout, TrendingUp, Users, ChevronRight } from "lucide-react";
...
// import DailyFocusModal from "../components/DailyFocusModal"; // [REMOVED] Daily Focus
...
export default function Dashboard() {
  ...
  const [viewDate, setViewDate] = useState(startOfToday()); // Anchor for the picker
  const [selectedApptForActions, setSelectedApptForActions] = useState(null);

  // ✅ Daily Focus Logic
  const [showDailyFocus, setShowDailyFocus] = useState(false);

  const nextDays = useMemo(() => {
    // Show 21 days: 7 before viewDate, 14 after
    return Array.from({ length: 21 }).map((_, i) => addDays(subDays(viewDate, 7), i));
  }, [viewDate]);
...
`;
  ...
  // ---------- BUILD SUBTREE (selected root UID + its descendants) using structureNodes OR driverChain
  const subtreeUids = useMemo(() => {
    if (selectedRootUid === "__me__") return [firebaseUser.uid];

    // If no root selected (or "Tutta la mia struttura"), return ALL structureUsers
    if (!selectedRootUid) {
    ...
    // STRATEGY 1: Use structureNodes (Visual Tree)
    const nodes = structureNodes || [];

    // STRATEGY 2: Use driverChain from users (Robust Fallback)
    // If structureNodes is empty OR we just want to be safe, use driverChain.
    // The previous logic returned [root] if nodes.length === 0, which caused the "Empty Structure" bug!

    // Let's rely on structureUsers (which contains the downline fetched via array-contains)
    // We want all users in structureUsers whose driverChain includes 'root' OR is 'root'

    const descendants = (structureUsers || []).filter(u => {
      const uid = String(u.uid || u.id || "").trim();
      const rootId = String(root || "").trim();

      if (uid === rootId) return true; // Include self

      const chain = (u.driverChain || []).map(c => String(c).trim());
      return chain.includes(rootId);
    });

    const foundUids = descendants.map(u => u.uid || u.id).filter(Boolean);

    // If we found people via chain, return them.
    if (foundUids.length > 0) {
      return [...new Set(foundUids)];
    }

    // Fallback to structureNodes graph logic if driverChain didn't give results (unlikely if structureUsers is populated)
    if (nodes.length > 0) {
    ...
    <main className="main dashboard-page">
      ...
      <section className={"cards-wrapper" + (activeTab !== "personal" ? " tab-hidden-mobile" : "")}>
        ...
        <div className="cards-grid cards-grid-personal">
          ...
          {/* --- ESEGUITI --- */}
          {/* TOTAL ESEGUITI */}
          <div className="card card-highlight" style={{ borderColor: 'var(--success)', background: 'rgba(16, 185, 129, 0.05)' }}>
            <div className="card-header">
          ...
          {/* MONTH ESEGUITI */}
          <div className="card card-secondary" style={{ borderColor: 'var(--success)', background: 'rgba(16, 185, 129, 0.05)' }}>
            <div className="card-header">
              <div className="card-title" style={{ color: 'var(--success)' }}>
                <Calendar size={14} className="card-icon" />
                Mese Eseguiti
      ...
      <section className={"cards-wrapper" + (activeTab !== "structure" ? " tab-hidden-mobile" : "")}>
        ...
        <div className="cards-grid cards-grid-structure">
          ...
          {/* ROW 2: ESEGUITI */}
          <div className="card card-structure" style={{ borderColor: 'var(--success)', background: 'rgba(16, 185, 129, 0.05)' }}>
            <div className="card-header">
      ...
      <section className={"cards-wrapper" + (activeTab !== "history" ? " tab-hidden-mobile" : "")}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <h2 className="cards-section-title" style={{ marginBottom: 0 }}>Agenda appuntamenti</h2>
        ...
        {apptsLoading ? (
          ...
          <div className="agenda-hybrid-container" style={{ marginTop: 20 }}>
            {/* 1. Navigation & Day Picker */}
            <div style={{ marginBottom: 15, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              <div style={{ color: '#fff', fontWeight: 800, fontSize: 16 }}>
                {format(viewDate, 'MMMM yyyy', { locale: it })}
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <button
                  onClick={() => setViewDate(prev => subDays(prev, 7))}
                  className="btn-icon-soft"
                  style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(255,255,255,0.05)', color: '#94a3b8', border: 'none' }}
                ...
                <button
                  ...
                  className="btn-icon-soft"
                  style={{ padding: '0 12px', height: 32, borderRadius: 8, background: 'rgba(139, 92, 246, 0.1)', color: '#a78bfa', border: '1px solid rgba(139, 92, 246, 0.2)', fontSize: 11, fontWeight: 700 }}
                >
                  OGGI
                </button>
                <button
            ...
            <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 15, scrollbarWidth: 'none', marginBottom: 20 }}>
              {nextDays.map(d => {
                const isSel = isSameDay(d, selectedDate);
                const isToday = isSameDay(d, startOfToday());
                // Count appts for this day
                const count = structureStats.list.filter(a => {
                  const ms = toMillis(a.dataOra);
                  if (!ms) return false;
                  const s = String(a.stato || a.status || "").toLowerCase();
                  if (s.includes("da risentire") || s.includes("da richiamare") || s.includes("da_risentire") || s.includes("da_richiamare")) return false;
                  return isSameDay(new Date(ms), d);
                }).length;
                ...
                return (
                  <button
                    ...
                    style={{
                      minWidth: 55, height: 70, borderRadius: 12, border: '1px solid',
                      borderColor: isSel ? '#8b5cf6' : isToday ? 'rgba(139, 92, 246, 0.3)' : 'rgba(255,255,255,0.05)',
                      background: isSel ? 'rgba(139, 92, 246, 0.2)' : 'rgba(255,255,255,0.02)',
                      color: isSel ? 'white' : isToday ? '#a78bfa' : '#94a3b8',
                      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transition: '0.3s',
                      flexShrink: 0, cursor: 'pointer', position: 'relative'
                    }}
                  >
                    <span style={{ fontSize: 9, textTransform: 'uppercase', fontWeight: 700 }}>{format(d, 'eee', { locale: it })}</span>
                    <span style={{ fontSize: 18, fontWeight: 900 }}>{format(d, 'd')}</span>
                    {count > 0 && <div style={{ position: 'absolute', top: 6, right: 6, width: 6, height: 6, borderRadius: '50%', background: '#8b5cf6' }}></div>}
                  </button>
                );
              })}
            </div>
            ...
            {/* 2. Compact List */}
            {(() => {
              const listForAgenda = structureStats.list.filter(a => {
                const ms = toMillis(a.dataOra);
                if (!ms) return false;
                const s = String(a.stato || a.status || "").toLowerCase();
                if (s.includes("da risentire") || s.includes("da richiamare") || s.includes("da_risentire") || s.includes("da_richiamare")) return false;
              ...
              if (listForAgenda.length === 0) {
                return <div style={{ textAlign: "center", padding: 40, border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 20, color: '#475569' }}>Nessun appuntamento per oggi.</div>;
              }

              // Sort by time
              const sorted = [...listForAgenda].sort((a, b) => (toMillis(a.dataOra) || 0) - (toMillis(b.dataOra) || 0));

              return (
                <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ padding: '12px 16px', background: 'rgba(139, 92, 246, 0.1)', fontSize: 11, fontWeight: 800, color: '#a78bfa', textTransform: 'uppercase' }}>
                  ...
                  {sorted.map((a, i) => {
                    ...
                    return (
                      <div
                        key={a.id || i}
                        onClick={() => setSelectedApptForActions(a)}
                        style={{
                          padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: i === sorted.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.05)', cursor: 'pointer'
                        }}
                      >
                        <div style={{ fontSize: 13, fontWeight: 900, width: 45, color: '#fff' }}>{hh}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: '#f8fafc' }}>
                            {(a.nome || "") + " " + (a.cognome || "")}
                          </div>
                          <div style={{ fontSize: 10, color: (statusType === 'positive' ? '#22c55e' : statusType === 'negative' ? '#ef4444' : '#94a3b8'), fontWeight: 600, textTransform: 'uppercase' }}>
                            {a.stato || (statusType === 'positive' ? 'Esito Positivo' : statusType === 'negative' ? 'Esito Negativo' : 'In programma')}
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                          {a.telefono && isMe && (
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); window.location.href = `tel:${a.telefono}`; }}
                              style={{ width: 32, height: 32, borderRadius: 10, background: 'rgba(34, 197, 94, 0.1)', color: '#22c55e', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                            >
                              <Phone size={14} />
                            </button>
                          )}
        ...
        {apptsLoading ? (
          ...
          <div className="agenda-wrapper" style={{ marginTop: 20 }} key={activeTab}>
            {(() => {
              ...
              // 2. Sort strictly by date DESCENDING (newest first)
              ...
              return (
                <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
                  {sorted.map((a, i) => {
                    const ms = toMillis(a.dataOra);
                    const dd = ms ? new Date(ms).toLocaleDateString("it-IT", { day: '2-digit', month: '2-digit' }) : "??/??";
                    const hh = ms ? new Date(ms).toLocaleTimeString("it-IT", { hour: '2-digit', minute: '2-digit' }) : "--:--";
                    const isMe = a.uid && firebaseUser?.uid && a.uid === firebaseUser.uid;
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 45 }}>
                          <span style={{ fontSize: 13, fontWeight: 900, color: '#fff' }}>{hh}</span>
                          <span style={{ fontSize: 8, color: '#64748b', fontWeight: 700 }}>{dd}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: '#f8fafc' }}>
                            {(a.nome || "") + " " + (a.cognome || "")}
                          ...
                          <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase' }}>
                            {isMe ? "Tu" : "Collab."} • {a.stato || "Da richiamare"}
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                          {a.telefono && isMe && (
      ...
      {/* QUICK ACTION SHEET (MIX LABS & DASHBOARD) */}
      {selectedApptForActions && (
        <div style={{
          position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 9999,
          display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)',
          padding: '20px'
        }} onClick={() => setSelectedApptForActions(null)}>
          <div
            style={{
              width: '100%', maxWidth: 450, background: '#1e293b', borderRadius: 30,
              padding: '30px 20px', position: 'relative', animation: 'zoomIn 0.3s ease-out',
          ...
          >
            ...
            <button
              onClick={() => setSelectedApptForActions(null)}
              style={{ position: 'absolute', right: 20, top: 20, background: 'rgba(255,255,255,0.05)', border: 'none', color: '#94a3b8', width: 32, height: 32, borderRadius: '50%', cursor: 'pointer' }}
            >✕</button>

            <div style={{ textAlign: 'center', marginBottom: 25 }}>
              <div style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>
                {(selectedApptForActions.nome || "") + " " + (selectedApptForActions.cognome || "")}
              </div>
              <div style={{ fontSize: 13, color: '#8b5cf6', fontWeight: 700, textTransform: 'uppercase', marginTop: 4 }}>
                {selectedApptForActions.stato || "In programma"}
              </div>
              <div style={{ fontSize: 12, color: '#64748b', marginTop: 8 }}>
                {toMillis(selectedApptForActions.dataOra) ? format(new Date(toMillis(selectedApptForActions.dataOra)), 'eeee d MMMM • HH:mm', { locale: it }) : ''}
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {selectedApptForActions.telefono && (selectedApptForActions.uid === firebaseUser?.uid) && (
                <button
                  onClick={() => { window.location.href = `tel:${selectedApptForActions.telefono}`; setSelectedApptForActions(null); }}
                  style={{ padding: '16px', borderRadius: 20, background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.2)', color: '#22c55e', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer' }}
                >
                  <Phone size={20} /> Chiama
                </button>
              )}
              <button
                onClick={() => { openView(selectedApptForActions); setSelectedApptForActions(null); }}
                style={{ padding: '16px', borderRadius: 20, background: 'rgba(139, 92, 246, 0.1)', border: '1px solid rgba(139, 92, 246, 0.2)', color: '#a78bfa', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer', gridColumn: (selectedApptForActions.telefono && selectedApptForActions.uid === firebaseUser?.uid) ? 'auto' : 'span 2' }}
              >
                <Eye size={20} /> Vedi Tutto
              </button>
            </div>

            <div style={{ marginTop: 12 }}>
              {(selectedApptForActions.uid === firebaseUser?.uid) && (
                <button
                  onClick={() => { openEdit(selectedApptForActions); setSelectedApptForActions(null); }}
                  style={{ width: '100%', padding: '18px', borderRadius: 20, background: '#8b5cf6', border: 'none', color: 'white', fontWeight: 800, fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.05em', cursor: 'pointer', boxShadow: '0 10px 20px rgba(139, 92, 246, 0.3)' }}
                >
                  SCRIVI ESITO / MODIFICA
                </button>
              )}
            </div>
          </div>
              ...
              <div className="crm-modal-body">
                ...
                <div className="field" style={{ marginTop: 12 }}>
                  ...
                  <div className="input" style={{ minHeight: 120, whiteSpace: "pre-wrap", lineHeight: 1.45, opacity: 0.95 }}>
                    {(() => {
                      const isMe = viewAppt?.uid === firebaseUser?.uid;
                      if (!isMe) return "Informazione riservata.";
                      return String(viewAppt?.note || viewAppt?.descrizione || "").trim() || "Nessuna nota inserita.";
                    })()}
                  </div>
                </div>
              </div>
