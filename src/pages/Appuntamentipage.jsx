...
export default function AppuntamentiPage() {
  ...
  // const [isBusyMode, setIsBusyMode] = useState(false);  <-- RIMOSSO, ora derivato da gridMode

  // [NEW] Busy Mode logic (Grid)
  const [gridMode, setGridMode] = useState("events"); // "events" | "busy"
  const isBusyMode = gridMode === "busy"; // ✅ DERIVATO
  ...
  const busySlotsRef = useRef(busySlots); // Ref per accesso nei listener/callback

  useEffect(() => { busySlotsRef.current = busySlots; }, [busySlots]);
  ...
  // ✅ SALVATAGGIO SU FIRESTORE (Helper)
  const saveBusyWeekToFirestore = async (wk, weekData) => {
    if (!firebaseUser?.uid || !wk) return;
    try {
      const docRef = doc(db, "users", firebaseUser.uid, "busy_slots", wk);
      // Sovrascriviamo l'intera mappa della settimana per evitare incongruenze
      await setDoc(docRef, weekData || {});
      // console.log("Salvataggio busy slots OK:", wk);
    } catch (e) {
      console.error("Errore salvataggio busy slots:", e);
    }
  };
  ...
  // ✅ Modifica SOLO LOCALE (per drag veloce)
  const setSlotBusyLocal = (wk, sk, val) => {
    setBusySlots(prev => {
  ...
  const toggleSlotBusy = (wk, sk) => {
    setBusySlots(prev => {
      ...
      // Save side-effect
      saveBusyWeekToFirestore(wk, newWeekObj);
  ...
  // ✅ LISTEN FIRESTORE PER BUSY SLOTS DELLA SETTIMANA
  useEffect(() => {
    if (!firebaseUser?.uid || !weekKey) return;

    // Ascolta documento users/{uid}/busy_slots/{weekKey}
    const unsub = onSnapshot(doc(db, "users", firebaseUser.uid, "busy_slots", weekKey), (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        setBusySlots(prev => ({ ...prev, [weekKey]: data }));
      } else {
        // Se non esiste, pulisci la settimana corrente (magari era stata cancellata altrove)
        setBusySlots(prev => {
          if (!prev[weekKey]) return prev; // già vuoto
          const next = { ...prev };
          delete next[weekKey];
          return next;
        });
        ...
        <div className="premium-modal-backdrop" id="modalViewAppointment">
          <div className="premium-modal">
            ...
            <div className="premium-modal-body">
              ...
              {(() => {
                const isMe = selectedAppointment.uid === myUid;
                if (!isMe) return null;
                return (
                  <>
                    <div className="view-row">
                    ...
                    <div className="view-row">
                      <div className="view-label">Note</div>
              ...
              {!selectedAppointment.uid === myUid && (
                <div className="view-row">
                  <div className="view-label">Info</div>
                  <div className="view-value" style={{ color: 'var(--text-muted)', fontStyle: 'italic' }}>
                    Informazioni di contatto riservate.
                  </div>
                </div>
              )}
