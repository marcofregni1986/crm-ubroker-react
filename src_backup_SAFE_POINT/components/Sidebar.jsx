...
const baseNavItems = [
  ...
  { to: "/profilo", label: "Profilo", icon: User }, // ✅ Added Profilo
...
export default function Sidebar({
  ...
  /* State for Digital Business Card */
  const [showBusinessCard, setShowBusinessCard] = React.useState(false);
  ...
  return (
    <aside className={"sidebar" + (sidebarOpen ? " open" : "")}>
      ...
      {/* ✅ USER BOX */}
      <div className="user-box relative group">
        <div className="flex items-center gap-3">
          <div className="user-avatar">{loading ? "…" : initial}</div>
          <div className="flex-1 min-w-0">
            <div className="user-info-name truncate">{loading ? "Caricamento…" : name}</div>
            <div className="user-info-sub truncate">
              {loading ? "Tel: —" : phone ? `Tel: ${phone}` : "Tel: —"}
            </div>
          </div>
          {/* QR Trigger Button - Only visible if not loading */}
          {!loading && (
            <button
              onClick={() => setShowBusinessCard(true)}
              className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-slate-800/50 rounded-lg transition-colors ml-1"
